...
export default class Message extends Component {
  ...
  paramsShow = (id, title, type) => {
    if (type == "push") {
      //push跳转
      this.props.history.push(`/21/home/message/detail/${id}/${title}`)
    } else {
      //replace跳转
      this.props.history.replace(`/21/home/message/detail/${id}/${title}`)
    }
  }
      ...
      <div>
        ...
        <ul>
          {messageArr.map(({ id, title }) => (<li key={id} style={{ display: 'flex', gap: "10px" }}>
            <Link to={`/21/home/message/detail/${id}/${title}`}>{title}(replace)</Link>
        ...
        {/* <ul>
          {messageArr.map(({ id, title }) => (<li key={id} style={{ display: 'flex', gap: "10px" }}>
          <Link to={{ pathname: `/21/home/message/detail`, state: { id, title } }}>{title}</Link>
        ...
        <hr />
        {/* params */}
        <Route path="/21/home/message/detail/:id/:title" component={Detail}></Route>
        {/* state */}
        {/* <Route path="/21/home/message/detail" component={Detail}></Route> */}
      </div>
    )
  }
}
